...
public class HealthLevelProvider
implements NumericValueProvider<HealthLevelProvider> {
    ...
    public float getValue(LivingEntity entity) {
        float value = entity.m_21223_();
        if (this.missing) {
            value = entity.m_21233_() - value;
        }
        if (this.percentage) {
            value /= entity.m_21233_();
        }
        return value;
    }
    ...
    public MutableComponent getConditionTooltip(SkillBonus.Target target, NumericValueCondition.Logic logic, Component bonusTooltip, float requiredValue) {
        ...
        String pointsKey = (String)key + ".point";
    ...
    public NumericValueProvider.Serializer getSerializer() {
        return (NumericValueProvider.Serializer)PSTNumericValueProviders.HEALTH_LEVEL.get();
    }
    ...
    implements NumericValueProvider.Serializer {
        ...
        public void serialize(JsonObject json, NumericValueProvider<?> provider) {
            if (!(provider instanceof HealthLevelProvider)) {
                throw new IllegalArgumentException();
            }
            HealthLevelProvider aProvider = (HealthLevelProvider)provider;
